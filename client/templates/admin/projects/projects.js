...
Template.list_projects.events({
    'submit .searchForm': function(){
        ...
        var imei = event.target.imei.value;
        var meid = event.target.meid.value;
        var itemid = event.target.itemid.value;
        var fromDate = event.target.fromDate.value;
        var toDate = event.target.toDate.value;
       
console.log("in here");
        if(imei=="" && meid == "" && itemid == "" && fromDate == "" && toDate == ""){

            FlashMessages.sendError("Select Atleast one Criteria!!");
             return false;
          //  Router.go('/admin/projects');
        } else {
var str = "imei="+imei+"&meid="+meid+"&itemid="+itemid+"&fromDate="+fromDate+"&toDate="+toDate;
           Router.go('/admin/projects/search?'+str);
           return false;
        }   
